...
class InventoryAgent:
    @staticmethod
    def _preferred_warehouse_name(user_profile):
    ...
    @staticmethod
    def _choose_warehouse(warehouses, preferred_name, offset):
        if not warehouses:
            return None
        if preferred_name:
    ...
    @staticmethod
    def _purchase_limit(stock_count, promotion_tag):
        if stock_count <= 3:
            return 1
        if stock_count <= 10:
            return 2
    ...
    def filter_stock(self, products, user_profile=None):
        ...
        preferred_warehouse_name = self._preferred_warehouse_name(user_profile)
        urgency = user_profile.get("urgency", "normal")
        fulfillment_preference = user_profile.get("fulfillment_preference", "standard")
        ...
        for product in products:
            ...
            for index, item in enumerate(product.get("search_results", [])):
                warehouse = self._choose_warehouse(warehouses, preferred_warehouse_name, index)
                warehouse_name = warehouse.get("name") if warehouse else "默认仓"
                warehouse_stock = warehouse.get("stock", 0) if warehouse else 0

                stock_count = max(0, warehouse_stock - index * 2)
                shipping_days = item.get("shipping_days", 0)
                if preferred_warehouse_name and warehouse_name == preferred_warehouse_name and stock_count > 0:
                    shipping_days = max(1, shipping_days - 1)

                fulfillment_type = "现货"
                presale_days = 0
                if stock_count == 0 and warehouse_total > 0:
                    stock_count = max(1, warehouse_total // max(len(product.get("search_results", [])), 1) - index * 2)
                    fulfillment_type = "调货"
                    shipping_days += 2
                ...
                purchase_limit = self._purchase_limit(stock_count, product.get("promotion_tag", ""))

                enriched_item = item.copy()
                enriched_item["shipping_days"] = shipping_days
                enriched_item["stock_count"] = stock_count
                enriched_item["stock_status"] = stock_status
                enriched_item["fulfillment_type"] = fulfillment_type
                enriched_item["presale_days"] = presale_days
                enriched_item["purchase_limit"] = purchase_limit
                enriched_item["fulfillment_warehouse"] = warehouse_name
                enriched_item["estimated_delivery"] = f"{shipping_days} 天内发货" if fulfillment_type != "预售" else f"预计 {presale_days} 天后发货"
